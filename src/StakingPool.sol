...
contract StakingPool {
    ...
    function getPoolByID(
        uint256 _id
    ) external view returns (PoolDataReturnedType memory _pool) {
    ...
    function stake(uint256 _poolID, uint256 _amount) external {
        ...
        p.stakerRewardPerSec[msg.sender] = _calculateRewardperSecond(
            _poolID,
            p.stakersBalances[msg.sender]
        );
    ...
    function _calculateRewardperSecond(
        ...
        _rewardPerSecond =
            (_stakedAmount * pools[_poolID].rewardRate) /
            secInDay;
    }
