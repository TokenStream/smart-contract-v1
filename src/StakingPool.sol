...
contract StakingPool {
    ...
    function getPoolByID(uint256 _id) external view returns (PoolDataReturnedType memory _pool) {
        _pool = PoolDataReturnedType(
            pools[_id].totalStakers, pools[_id].totalStaked, pools[_id].rewardReserve, pools[_id].rewardRate
        );
    }
    ...
    function _calculateRewardperSecond(uint256 _poolID, uint256 _stakedAmount)
        private
        view
